...
Friend Class clsALG
   ...
   Sub OPRrwert(ByRef IBASF As Short, ByRef BUF As String, ByRef r() As Single, ByRef istat As Short)
      ...
      For j = 1 To MessgNwe
         ...
         If Mid(BUF, (j - 1) * 14 + 1, 1) <> "+" And Mid(BUF, (j - 1) * 14 + 1, 1) <> "-" Then
            imsg = MsgBox(Texxt(3507) & Chr(13) & Texxt(1999), 4, Texxt(2000))
            If imsg = 7 Then
               istat = -1
            Else
            ...
            Exit Sub
         End If
         Smes = Val(Mid(BUF, (j - 1) * 14 + 1, 7))
         Sref = Val(Mid(BUF, (j - 1) * 14 + 8, 7))
         ' MsgBox Sref & "   " & Smes
         r(j) = Smes / Sref
         'MsgBox r(j)
      Next j
      ...
      '
      Nwp = MessgNwp
      Call RefWsolIst(Nwp, r, MessgWist, MessgWsol)
   ...
   Sub D38rwert(ByRef IBASF As Short, ByRef BUF As String, ByRef r() As Single, ByRef istat As Short)
      ...
      '
      For j = 1 To 16
         r(j) = Val(Mid(BUF, (j - 1) * 7 + 5, 7)) * 0.001
      Next j
      '
      '       Ist-Wellenlängen-Korrektur
      '
      'Call RefWsol_ist(messgnwe, r, MessgWist, MessgWsol)
   ...
   Sub OPQrwert(ByRef IBASF As Short, ByRef BUF As String, ByRef r() As Single, ByRef istat As Short)
      ...
      'buf = BufFiltr(buf)
      For j = 1 To 16
         If Mid(BUF, (j - 1) * 12 + 1, 1) <> "+" And Mid(BUF, (j - 1) * 12 + 1, 1) <> "-" Then
            imsg = MsgBox(Texxt(3507) & Chr(13) & Texxt(1999), 4, Texxt(2000))
            If imsg = 7 Then
               istat = -1
            Else
            ...
            Exit Sub
         End If
         Smes = Val(Mid(BUF, (j - 1) * 12 + 1, 6))
         Sref = Val(Mid(BUF, (j - 1) * 12 + 7, 6))
         'MsgBox Sref & "   " & Smes
         r(j) = Smes / Sref
         'MsgBox r(j)
      Next j
   ...
   Sub OPOrwert(ByRef IBASF As Short, ByRef BUF As String, ByRef r() As Single, ByRef istat As Short)
      ...
      For j = 1 To 16
         If Len(BUF) <> 144 Then
            ...
            Exit Sub
         End If
         Smes = Val(Mid(BUF, (j - 1) * 4 + 1, 4))
         Sref = Val(Mid(BUF, (j - 1) * 4 + 73, 4))
         'MsgBox Sref & "   " & Smes
         r(j) = Smes / Sref
         'MsgBox r(j)
      Next j
   ...
   Sub OPRMes(ByRef imess As Short, ByRef kw As Short, ByRef r() As Single, ByRef istat As Short, ByRef mscMES As SerialPort)
      ...
      Dim IBASF As Short
      Kwr = 8
      'UPGRADE_WARNING: Lower bound of array STVWI was changed from 1 to 0. Click for more: 'ms-help://MS.VSExpressCC.v80/dv_vsexpcc/scripts/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
      ReDim STVWI(Kwr)
      'UPGRADE_WARNING: Lower bound of array Ccrm was changed from 1 to 0. Click for more: 'ms-help://MS.VSExpressCC.v80/dv_vsexpcc/scripts/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
      ReDim Ccrm(Kwr)
      'UPGRADE_WARNING: Lower bound of array CCRN was changed from 1 to 0. Click for more: 'ms-help://MS.VSExpressCC.v80/dv_vsexpcc/scripts/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
      ReDim CCRN(Kwr)
      ...
      For i = 1 To Kwr
         ...
         End If
      Next i
      Call GetComBuf(resp, stv, BUF, 10, 5, 10, 10, 3500, 1, "", 1, "?", mscMES, istat)
      ' MsgBox BUF
      If istat <> NuWrt Then Exit Sub
      IBASF = 0
   ...
   Sub OPRini(ByRef istat As Short)
      ...
      For kw = 1 To MessgKM
         nkw = (kw - 1) * MessgNwe
         For i = 1 To MessgNwe
            MessgWist(nkw + i) = Val(Mid(BUF, (i - 1) * 8 + 1, 7))
         Next i
      Next kw
   End Sub
